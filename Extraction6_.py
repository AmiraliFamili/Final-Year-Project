...
def build_dataset_storage_paths(dataset_dir: str | Path) -> dict[str, Path]:
    ...
    return {
        ...
        "sample_manifest": meta / "sample_manifest.jsonl",
    }
    ...
    if label_column:
        if not paths["labels"].exists():
            ...
            if labels.dtype.kind in ("U", "S", "O"):
                ...
                # For numeric labels, save identity mapping
                unique_labels = np.unique(labels)
                np.save(paths["label_codes"], unique_labels)
                # labels stay as original ints
        ...
        with open(manifest_path, 'w') as f:
            for start in range(0, n_samples, batch_size):
                ...
                for idx in range(start, end):
                    f.write(json.dumps({"sample_id": int(idx), "batch_start": start, "batch_end": end, "batch_hash": batch_hash}) + "\n")
    ...
    if not skipped and paths["completed"].exists():
        ...
        if int(completed.sum()) == n_samples:
            ...
            if meta_path.exists():
                ...
                head_match = prov.get("head_hash") == current_prov["head_hash"]
                tail_match = prov.get("tail_hash") == current_prov["tail_hash"]
                sample_count_match = meta.get("dataset", {}).get("samples") == n_samples
                text_col_match = meta.get("dataset", {}).get("text_column") == column
                pooling_match = meta.get("extraction", {}).get("pooling") == pooling
                max_len_match = meta.get("extraction", {}).get("max_length") == max_length
                batch_match = meta.get("extraction", {}).get("batch_size") == batch_size

                core_match = sample_count_match and text_col_match and pooling_match and max_len_match and batch_match and (head_match or tail_match)

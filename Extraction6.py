...
"""Deterministic hidden-state extraction pipeline (corrected v6, with data integrity safeguards).
...
def build_dataset_storage_paths(dataset_dir: str | Path) -> dict[str, Path]:
    ...
    return {
        ...
        "sample_ids": meta / "sample_ids.npy",
        ...
        "sample_manifest": meta / "sample_manifest.jsonl",   # <-- add this line
    }
...
def flush_array(array: np.ndarray) -> None:
    ...
    if not isinstance(array, np.memmap):
        return
    # First flush the internal buffers
    array.flush()
    # Then open the file and fsync it to ensure durability
    path = Path(array.filename)
    if path.exists():
        fd = os.open(str(path), os.O_RDWR)  # we have write permission
        try:
            os.fsync(fd)
        finally:
            os.close(fd)
    # Finally fsync the directory entry to guarantee the file is visible
...
def dataset_output_is_complete(output_dir: str | Path, *, expected_signature: Mapping[str, Any] | None = None) -> bool:
    ...
    try:
        ...
        if not _metadata_matches_completion_signature(meta, expected_signature):
            return False
        # Check that labels and sample_ids exist if expected
...
class RuntimeReporter:
    ...
    def __init__(self, dataset_name: str, total: int, initial_completed: int, show_verbose: bool, show_info: bool, show_critical: bool, show_debug: bool, events_path: Path, runtime_path: Path, context: Mapping[str, Any]):
        ...
        self.peak_rss_gb = 0.0; self.minimum_available_ram_gb = math.inf; self.anomaly_count = 0; self.slow_batch_count = 0; self._events_file = None
    ...
    def maybe_runtime_report(self, force: bool = False):
        ...
        if len(self.batch_durations) >= 5 and self.rolling_median > 0:
            ...
            if anomaly: self.anomaly_count += 1; self.slow_batch_count += 1
...
def ensure_auxiliary_files(
    ...
    """
    Check for and reconstruct missing auxiliary files using existing hidden states
    and the provided dataset object. Returns a summary of actions taken.
    If hidden_states.npy is missing, returns a skipped flag and does nothing.
    """
    actions = []
    ...
    # --- Sample IDs ---
    if not paths["sample_ids"].exists():
        sample_ids = np.arange(n_samples, dtype=np.int64)
        np.save(paths["sample_ids"], sample_ids)
        actions.append("created sample_ids.npy")
    ...
    if label_column:
        ...
        if not paths["labels"].exists():
            ...
            if labels.dtype.kind in ("U", "S", "O"):
                ...
                actions.append("created label_codes.npy and encoded labels.npy")
            else:
        ...
        elif paths["label_codes"].exists() and not paths["labels"].exists():
            actions.append("labels.npy missing but label_codes present; cannot reconstruct without original data")
    else:
        # No label column
        pass
    ...
    if not paths["integrity"].exists():
        # Recompute batch hashes from hidden_states.npy
    ...
    # --- Sample manifest (optional, per‑sample hash) ---
    if not paths.get("sample_manifest", Path()).exists():
        # We'll create a per‑batch manifest with sample indices and batch hash
        states = np.load(paths["states"], mmap_mode='r')
        manifest_path = paths["metadata_dir"] / "sample_manifest.jsonl"
        with open(manifest_path, 'w') as f:
            for start in range(0, n_samples, batch_size):
                end = min(start + batch_size, n_samples)
                data = np.asarray(states[start:end])
                batch_hash = hashlib.sha256(data.tobytes()).hexdigest()
        ...
        actions.append("created sample_manifest.jsonl")
...
def repair_dataset_auxiliaries(
    ...
    """
    Check for and repair missing auxiliary files for an existing dataset.
    If the dataset is complete and the only mismatch in metadata is missing new fields,
    it updates the metadata to match the current experiment.
    Returns a summary dict with keys:
        - 'actions': list of actions taken
        - 'skipped': bool (True if hidden_states missing)
    ...
    if not skipped and paths["completed"].exists():
        ...
        if int(completed.sum()) == n_samples:
            ...
            if meta_path.exists():
                ...
                # Check core fields match (ignore experiment_id/hyperparameter_hash for old metadata)
                core_match = (
                ...
                if core_match:
                    ...
                    if "dataset" not in meta:
                        meta["dataset"] = {}
                    meta["dataset"]["name"] = meta.get("dataset", {}).get("name") or Path(output_dir).parent.name
                    meta["dataset"]["text_column"] = column
                    meta["dataset"]["samples"] = n_samples
                    meta["dataset"]["fingerprint"] = fp
                    meta["dataset"]["provenance"] = dataset_signature(dataset, texts, column)
                    meta["dataset"]["columns"] = list(getattr(dataset, "columns", []))
                    meta["dataset"]["labels"] = {
                        "label_column": None,  # will be filled if labels exist
                        "labels_dtype": None,
                        "label_codes_path": str(paths["label_codes"]) if paths["label_codes"].exists() else None,
                    }
                    # Detect label column for metadata
                    for cand in ("labels", "label", "target", "emotion"):
                        if cand in dataset.columns:
                            meta["dataset"]["labels"]["label_column"] = cand
                            break
                    meta["dataset"]["sample_ids_path"] = str(paths["sample_ids"])
                    meta["extraction"] = {
                        "pooling": pooling,
                    ...
                    actions.append("updated extraction.json with current experiment fields")
                    metadata_updated = True
                else:
                    actions.append("metadata core fields differ; metadata not updated")
            else:
...
def extract_dataset(model: torch.nn.Module, tokenizer: Any, dataset: Any, text_column: str | None, output_dir: str | Path, batch_size: int, pooling: str = DEFAULT_POOLING, max_length: int | None = DEFAULT_MAX_LENGTH, storage_dtype: np.dtype = np.float32, device: torch.device | None = None, model_name: str | None = None, dataset_name: str | None = None, model_snapshot: str | Path | None = None, auto_batch_size: bool = False, max_batch_size: int | None = None, flush_every_batches: int = DEFAULT_FLUSH_EVERY_BATCHES, flush_every_seconds: float = DEFAULT_FLUSH_SECONDS, show_verbose: bool = True, show_info: bool = True, show_critical: bool = True, show_debug: bool = False, experiment_id: str = DEFAULT_EXPERIMENT_ID, hyperparameter_hash: str | None = None) -> dict[str, Any]:
    ...
    if label_column:
        ...
        if paths["labels"].exists():
            ...
            if labels_mmap.shape != (n_samples,) or labels_mmap.dtype != labels.dtype:
                raise ValueError("Existing labels.npy incompatible")
        else:
            labels_mmap = np.lib.format.open_memmap(paths["labels"], mode="w+", dtype=labels.dtype, shape=(n_samples,))
            labels_mmap[:] = labels
            flush_array(labels_mmap)
    ...
    if paths["sample_ids"].exists():
        ...
        if sample_ids.shape != (n_samples,):
            raise ValueError("Existing sample_ids.npy incompatible")
    else:
        sample_ids = np.arange(n_samples, dtype=np.int64)
        np.save(paths["sample_ids"], sample_ids)
    ...
    if completed_count == n_samples:
        # Try to repair metadata if core fields match
        if paths["metadata"].exists():
            try:
                ...
                if core_match:
                    # Update metadata to current experiment fields
                    meta["status"] = "complete"
                    meta["experiment_id"] = experiment_id
                    meta["hyperparameter_hash"] = hyperparameter_hash
                    meta["dataset"]["text_column"] = column
                ...
                try:
                    ...
                    if successful_batches % flush_every_batches == 0 or now-last_flush >= flush_every_seconds or end >= n_samples:
                        ...
                        if labels_mmap is not None:
                            flush_array(labels_mmap)
                        # fsync directory to ensure file entries are durable
                    ...
                    # Remove temp file after successful flush
                    if temp_path.exists():
                        temp_path.unlink()

            ...
            while position < n_samples:
                ...
                try:
                    ...
                    if successful_batches % flush_every_batches == 0 or now-last_flush >= flush_every_seconds or end >= n_samples:
                        reporter.set_stage("durable_flush"); ft = time.perf_counter(); flush_array(states); flush_array(completed); flush_s = time.perf_counter()-ft; totals["flush"] += flush_s; last_flush = time.perf_counter(); save_json(paths["runtime"], {**context, "status": "running", "position": end, "completed": int(completed.sum()), "updated_at": time.time()})

...
def get_available_models(extraction_mod=None):
    """
    Return a list of (model_name, family, params) from the extraction registry.
    """
    if extraction_mod is None:
        extraction_mod = ModuleFactory.extraction(MasterConfig())
    if hasattr(extraction_mod, "MODEL_REGISTRY"):
        return [
            (m.name, m.family, m.parameter_billions) for m in extraction_mod.MODEL_REGISTRY
        ]
    return []


def resolve_model_name(raw: str, available: Sequence[tuple]) -> str:
    """
    Resolve a user-provided model name to a full registry name if possible.

    Examples:
        "Qwen2-1.5B" -> "Qwen/Qwen2-1.5B" if unique match
        "bert"       -> "google-bert/bert-base-uncased" if unique match
    """
    raw = raw.strip()
    if not raw:
    ...
    matches = [name for name, _, _ in available if raw.lower() in name.lower()]
    if len(matches) == 1:
        return matches[0]
    if len(matches) > 1:
        ...
        try:
            ...
            return matches[0]
    # If no match, return the raw string (will likely fail later)
    print(f"Warning: Model '{raw}' not found in registry. Using it as-is.")
    return raw
...
class EmotionProbePipeline:
    ...
    def run_probes(self) -> pd.DataFrame:
        ...
        try:
            ...
            # Incomplete run – but progress has been saved. We can still use partial results.
...
class InteractiveApp:
    ...
    def run(self) -> dict[str, Any]:
        ...
        # Load the extraction module to get the model registry
        extraction_mod = ModuleFactory.extraction(MasterConfig())
        available_models = get_available_models(extraction_mod)
        if not available_models:
            # Fallback to manual entry if no registry found
            model = self.ask("Model", "google-bert/bert-base-uncased")
        else:
            print("\nAvailable models:")
            for i, (name, family, params) in enumerate(available_models, 1):
                print(f"  {i:2d}. {name}  ({family}, {params:.3f}B)")
            ...
            if raw == "":
                ...
                if 0 <= idx < len(available_models):
                    model = available_models[idx][0]
                else:
                    print("Invalid selection, using default.")
                    model = "google-bert/bert-base-uncased"
            else:
                # User typed a name; resolve to full registry name if possible
                model = resolve_model_name(raw, available_models)
        ...
        # Ask if extraction should be run if missing
        ...
        # Pass extract_if_missing to run
        return pipeline.run(extract_if_missing=extract_if_missing)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "run":
        ...
        # CLI should allow extraction if missing only if flag set
